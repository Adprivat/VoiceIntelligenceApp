    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-lg mx-4 max-h-[85vh] bg-[var(--background)] border border-[var(--border)] rounded-xl shadow-2xl fade-in flex flex-col">
        ...
        <div className="p-4 space-y-5 overflow-y-auto flex-1">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-[var(--foreground)]">
              {t("settings.microphone.label", lang)}
            </label>
            <div className="flex gap-2">
              <select
                value={settings.selectedMicrophone}
                onChange={(e) => updateField("selectedMicrophone", e.target.value)}
                className="flex-1 px-3 py-2 rounded-lg bg-[var(--card)] border border-[var(--border)] text-sm text-[var(--foreground)] focus:outline-none focus:border-[var(--primary)] transition-colors"
              >
                <option value="">{t("settings.microphone.default", lang)}</option>
                {microphones.map((mic) => (
              ...
              <button
                ...
                className="px-3 py-2 rounded-lg bg-[var(--card)] border border-[var(--border)] hover:bg-[var(--card-hover)] text-xs text-[var(--muted)] transition-colors"
                title={t("settings.microphone.refresh", lang)}
              >
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M1 4v6h6" />
        ...
        <div className="flex justify-between p-4 border-t border-[var(--border)] shrink-0">
          <button
            onClick={async () => {
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-red-500/30 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-sm font-medium transition-colors"
          >
